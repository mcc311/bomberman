...
with open(r'Body\Object\Bomb\bomb_adrs.json') as load:
    bomb_adrs = json.load(load)
...
class Bomb:
    ...
    def draw(self,win):
        for i in range(1):
            img = pygame.image.load(bomb_adrs[str(self.timer//50*2 + i)])
            win.blit(img, (self.pos_x, self.pos_y))
            pygame.time.delay(1)
